// URLs for different word lists
  const JSON_URLS = {
  b1telcpt1: "https://script.google.com/macros/s/AKfycbx4gHC-bY0VmvCeZo1fnZ2myu9qdEIoJw0xCk0kjb-yFVinPHypN458aZDdlI1CEJM/exec",
  b1telcpt2: "https://script.googleusercontent.com/macros/echo?user_content_key=Gs_T1_BtJgo68Rq7DUrGxs1uKJH3uaXkCkAAG-VJlJTyeFLEEndTWIwgDxUFlxPsEnbhNpXqL5p-A_udEgZzOavWNpCykLr8m5_BxDlH2jW0nuo2oDemN9CCS2h10ox_1xSncGQajx_ryfhECjZEnALkkP5rhZLWK4mhfbdLHXykRb2__QFrBFG1jt2RdQ25xS94AGBZe3egWcJgiE_WTDETglXqkmc3b4QFRgnNx1W8cv7vWJ9jUQ&lib=MmEIQx_PXkRyMvxX9OmNoYFLDhaBgl_dE",
  b1telcpt3: "https://script.googleusercontent.com/macros/echo?user_content_key=-DqeJ4hRVzZQ6jg7GzNqXqau-YDeYAuUbIJSfCxdlec8ogcIlFMrDJHGJL_gOhTKjoKxohhDhQeRX8RbXMplgDwM6CyHTGmnm5_BxDlH2jW0nuo2oDemN9CCS2h10ox_1xSncGQajx_ryfhECjZEnENgxJCDFrjs6JyZ1GGG4RM390WSYlFJahGv7vxAesNVHcnZqINdAEhBXe369Q_rkTCCIDBzZyM04DQEGU-K7khKoSOdad6tfw&lib=MmEIQx_PXkRyMvxX9OmNoYFLDhaBgl_dE",
  b1telcpt4: "https://script.googleusercontent.com/macros/echo?user_content_key=X5GmhOorbYcFAO9AlOBaKeigyKoSTW3fsY_nKBLZH6yNSgHGmqEU7BW-sShIapltxLFruSb5AYWrB4wVzGmkJIPVhIyUWXSYm5_BxDlH2jW0nuo2oDemN9CCS2h10ox_1xSncGQajx_ryfhECjZEnJZ31fvYy2gsmZkeRqZLyZW-rzexs4eP9SbReR_KHkVIG4bpUFOqrAIxJMj1cgsZLrxKiHKDk848jFZLaVWSAKXiKlrA50qxHQ&lib=MmEIQx_PXkRyMvxX9OmNoYFLDhaBgl_dE"
};
  ...
  link.addEventListener("click", async function (event) {
    ...
    if (selectedOption) {
      ...
      localStorage.setItem("correctAnswerWordsCounter",
          correctAnswerWordsCounter);

      // UI'ı güncelle
      document.getElementById(
          "remainingWordsCountLearn").innerText = learnedWords;
      document.getElementById(
          "remainingWordsCountExercise").innerText = correctAnswerWordsCounter;
  ...
  "remainingWordsCountLearn").innerText = learnedWords;
  document.getElementById(
  "totalWordsCountExercise").innerText = totalWordsExercise;
  document.getElementById(
  "remainingWordsCountExercise").innerText = correctAnswerWordsCounter;
  ...
  kelimeListesi = kelimeListesi.filter(word =>
  !learnedWords.some(
  ...
  highlightedWord = almanca.replace(regex,
  `<span class="highlight">$1</span>`);
}

  document.getElementById("levelTagLearn").innerText = seviye || "N/A";
  document.getElementById(
  "wordLearn").innerHTML = `<span style="color: ${renk};">${highlightedWord}</span>`;
  ...
  let inProgressWords = JSON.parse(localStorage.getItem("inProgressWords"))
  || [];

  learnedWithExerciseWords = JSON.parse(
  ...
  kelimeListesiExercise = kelimeListesiExercise.filter(word =>
  ...
  document.getElementById(
  "exampleLearn").innerText = "You learned all of the words, go to exercise section.";

  buttonDer.style.visibility = "hidden";
  buttonDie.style.visibility = "hidden";
  buttonDas.style.visibility = "hidden";
  document.getElementById(
  "feedbackMessage").innerText = `You completed all exercise words! 🎉`;
}
  ...
  document.getElementById("progressLeft").style.opacity = counter >= 1 ? "1"
  : "0.5";
  document.getElementById("progressMiddle").style.opacity = counter >= 2 ? "1"
  : "0.5";
  document.getElementById("progressRight").style.opacity = counter >= 3 ? "1"
  : "0.5";
} else {
  // Default state
  ...
  if (!kelimeListesiExercise.length || currentExerciseIndex
  ...
  console.log(
  `'${currentExerciseIndex}' index böyleydi.`);
  console.log(
  `'${kelimeListesiExercise.length}' kelime listesi uzunlugu böyleydi.`);
  let inProgressWords = JSON.parse(localStorage.getItem("inProgressWords"))
  || [];
  let learnedWithExerciseWords = JSON.parse(
  localStorage.getItem("learnedWithExerciseWords")) || [];
  ...
  const renk = artikelRenk(artikel);
  document.getElementById(
  "correctAnswerField").innerHTML = `<span style="color: ${renk};">${artikel}</span>`;
  ...
  if (inProgressWords[inProgressIndex].counter === 3) {
  document.getElementById(
  "feedbackMessage").innerText = `This word: ${currentWord.almanca} added to learned list!🏆`;
  document.getElementById("feedbackMessage").style.color = "green";
  ...
  kelimeListesiExercise.splice(currentExerciseIndex + 8, 0,
  currentWord)[0]
} else {
  kelimeListesiExercise.splice(currentExerciseIndex + 12, 0,
  currentWord)[0]
}
  currentExerciseIndex++;
  if (currentExerciseIndex >= kelimeListesiExercise.length) {
  ...
  localStorage.setItem("learnedWithExerciseWords",
  ...
  document.getElementById(
  "feedbackMessage").innerText = `Upps! ⚠️ Rule: ${kural}`;
  ...
  console.log(
  `'${currentExerciseIndex}' index bu sayiya güncellendi.`);
  console.log(
  ...
  document.getElementById("buttonDer").addEventListener("click",
  function (event) {
  event.preventDefault(); // Sayfanın yukarı kaymasını engeller
  checkAnswer("der");
});

  document.getElementById("buttonDie").addEventListener("click",
  function (event) {
  event.preventDefault();
  checkAnswer("die");
});

  document.getElementById("buttonDas").addEventListener("click",
  function (event) {
  event.preventDefault();
  checkAnswer("das");
});
  ...
  let learnedWithLearnWords = JSON.parse(
  localStorage.getItem("learnedWithLearnWords")) || [];
  ...
  localStorage.setItem("learnedWithLearnWords",
  ...
  "remainingWordsCountLearn").innerText = learnedWords;
  document.getElementById(
  "totalWordsCountLearn").innerText = initialTotalWords;
  ...
  "remainingWordsCountExercise").innerText = correctAnswerWordsCounter;
  document.getElementById(
  "totalWordsCountExercise").innerText = initialTotalWords;
  ...
  if (kelimeListesi.length === 0 || currentLearnIndex
  ...
  document.getElementById(
  "repeatButtonLearn").style.visibility = "hidden";
}
